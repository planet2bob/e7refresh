...
Originally intended for bluestacks on 1080p
Modified by planet2bob for scrcpy on 1440p
"""
...
def buy(bookmark):
    ...
    #         pos = pyautogui.locateOnScreen('screenshots/buycovenant.png',confidence=0.85)
    #     else:
    #         pos = pyautogui.locateOnScreen('screenshots/buycovenant.png',confidence=0.85)

    # pos_point = pyautogui.center(pos)
    # #click buy
    # click(pos_point[0]+rand_x, pos_point[1]+40+rand_y)
    # click(pos_point[0]+rand_x, pos_point[1]+40+rand_y)
        if (bookmark == 'covenant'):
            pos = pyautogui.locateOnScreen('screenshots/covenant.png',confidence=0.8)
        else:
            pos = pyautogui.locateOnScreen('screenshots/mystic.png',confidence=0.8)

        pos_point = pyautogui.center(pos)
        #click buy
        click(pos_point[0]+900+rand_x, pos_point[1]+35+rand_y)
        click(pos_point[0]+900+rand_x, pos_point[1]+35+rand_y)
    ...
    while(time.time() < (timeout_start + timeout)):
        if (bookmark == 'covenant'):
            Buy_button_pos=pyautogui.locateOnScreen('screenshots/buycovenantconfirm.png', confidence=0.6)
        else:
            Buy_button_pos=pyautogui.locateOnScreen('screenshots/buymysticconfirm.png', confidence=0.6)
...
RB_pos=pyautogui.locateOnScreen('screenshots/refresh.png',confidence=0.8)
...
while ((exit_flag == 0) and (time.time() < start_time+run_timeout)):
    ...
    Coven_pos=pyautogui.locateOnScreen('screenshots/covenant.png',confidence=0.8)
    Mystic_pos=pyautogui.locateOnScreen('screenshots/mystic.png',confidence=0.8)   
...
#Checks for mystic
    ...
    #Scroll down
    scroll_pt_x = random.randint(1400,1800)
    scroll_pt_y = random.randint(800,900)

    pyautogui.moveTo(scroll_pt_x, scroll_pt_y)
    pyautogui.mouseDown(button='left')
    pyautogui.moveTo(scroll_pt_x, scroll_pt_y - 500, 0.2)
    pyautogui.mouseUp(button='left')

    time.sleep(0.1)
    Coven_pos2=pyautogui.locateOnScreen('screenshots/covenant.png',confidence=0.8)
    Mystic_pos2=pyautogui.locateOnScreen('screenshots/mystic.png',confidence=0.8)
...
#When testing, pause to allow user to exit
    ...
    timeout_start = time.time()
    while(time.time() < (timeout_start + timeout)):
        Confirm_pos=pyautogui.locateOnScreen('screenshots/confirm.png', confidence=0.8)
        if (keyboard.is_pressed('q') == True):
            exit_flag = 1
            break
        if (Confirm_pos != None):
